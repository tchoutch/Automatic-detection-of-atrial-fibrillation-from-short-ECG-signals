...
if __name__ == '__main__':
    ...
    with open(os.path.join(path, 'result.txt'), 'w') as f:
        ...
        f.write('Test 5: Test data is random with different shapes - CNN model - Multicalss\n')
        test5 = predict_labels(ecg_leads,300,ecg_names,'CNN_V2',False)
        t.predToFile(f,test5)
        f.write('Test 5 finished\n')
        f.write('Test 6: Test data is random with different shapes - CNN model - Binary\n')
        test6 = predict_labels(ecg_leads,300,ecg_names,'CNN_V2',True)
        t.predToFile(f,test6)
        f.write('Test 6 finished\n')

        ecg_leads = [np.zeros((200000,)),np.zeros((100,)),np.ones((18000,))]
        ecg_names = ['A','B','C']
        f.write('Test 7: Test data is random with different shapes - LSTM model - Multicalss\n')
        test5 = predict_labels(ecg_leads,300,ecg_names,'LSTM_v1',False)
        t.predToFile(f,test5)
        f.write('Test 7 finished\n')
        f.write('Test 8: Test data is random with different shapes - LSTM model - Binary\n')
        test6 = predict_labels(ecg_leads,300,ecg_names,'LSTM_v1',True)
        t.predToFile(f,test6)
        f.write('Test 8 finished\n')
        f.write('Test 9: Test data is random with different shapes - CNN model - Multiclass\n')
        test7 = predict_labels(ecg_leads,300,ecg_names,'CNN_v1',False)
        t.predToFile(f,test7)
        f.write('Test 9 finished\n')
        f.write('Test 10: Test data is random with different shapes - CNN model - Binary\n')
        test8 = predict_labels(ecg_leads,300,ecg_names,'CNN_v1',True)
        t.predToFile(f,test8)
        f.write('Test 10 finished\n')
        f.write('Test 11: Test data is random with different shapes - CNN model - Multicalss\n')
        test5 = predict_labels(ecg_leads,300,ecg_names,'CNN_V2',False)
        t.predToFile(f,test5)
        f.write('Test 11 finished\n')
        f.write('Test 12: Test data is random with different shapes - CNN model - Binary\n')
        test6 = predict_labels(ecg_leads,300,ecg_names,'CNN_V2',True)
        t.predToFile(f,test6)
        f.write('Test 12 finished\n')
        f.close()
    f1.close()
    f2.close()